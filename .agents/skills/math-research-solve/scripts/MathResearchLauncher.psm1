    ...
    try {
        foreach ($candidate in $candidates) {
            ...
            try {
                ...
                $envelope = $text | ConvertFrom-Json -AsHashtable -Depth 64
...
function Parse-PromptV6Metadata {
    ...
    try {
        $policy = [string]$base.cycle_policy_json | ConvertFrom-Json -AsHashtable -Depth 64
        $tickets = [string]$base.initial_tickets_json | ConvertFrom-Json -AsHashtable -Depth 64
    }
...
function Read-CodexJsonLog {
    ...
    foreach ($line in [IO.File]::ReadLines($LiteralPath, [Text.UTF8Encoding]::new($false, $true))) {
        ...
        try { $event = $line | ConvertFrom-Json -AsHashtable -Depth 64 }
...
function Test-GoalReadyMarker {
    ...
    try { $marker = $Message | ConvertFrom-Json -AsHashtable -Depth 8 }
