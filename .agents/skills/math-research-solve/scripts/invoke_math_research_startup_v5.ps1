...
$env:PYTHONUTF8='1'
$engine=Join-Path $PSScriptRoot 'math_research_state_v10.py'
...
if($parsed.ok-and$parsed.data.classification-ceq'delegate_startup_v4'){
    & (Join-Path $PSScriptRoot 'invoke_math_research_startup_v4.ps1') -ProjectPath $ProjectPath -AuditMode $AuditMode -GoalStatus $GoalStatus
    if(-not $?){exit 1}
    exit 0
}
$raw.TrimEnd()
