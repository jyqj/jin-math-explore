...
function Import-CycleControllerBundle {
    $modulePath = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot 'MathResearchCycleLedger.psm1'))
    $cliPath = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot 'invoke_math_research_cycle.ps1'))
