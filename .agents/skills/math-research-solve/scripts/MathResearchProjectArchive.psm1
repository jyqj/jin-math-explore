...
function Read-ProjectJsonFile {
    ...
    try { $value = $text | ConvertFrom-Json -AsHashtable -Depth 64 } catch { throw "$Label cannot be decoded." }
...
function Read-ImportManifest {
    ...
    foreach ($line in [IO.File]::ReadLines($path, [Text.UTF8Encoding]::new($false, $true))) {
        ...
        try { $record = $line | ConvertFrom-Json -AsHashtable -Depth 20 } catch { throw "Invalid import manifest line $lineNumber." }
        $records += $record
    }
    return ,$records
}
...
function Publish-MathResearchProjectCheckpoint {
    ...
    Verify-MathResearchProjectArchive -ProjectDirectory $ProjectDirectory|Out-Null
    $context = Resolve-ProjectRunDirectory -ProjectDirectory $ProjectDirectory -RunDirectory $RunDirectory
    $launcherModule = Join-Path $PSScriptRoot 'MathResearchLauncher.psm1'
    $cycleModule = Join-Path $PSScriptRoot 'MathResearchCycleLedger.psm1'
