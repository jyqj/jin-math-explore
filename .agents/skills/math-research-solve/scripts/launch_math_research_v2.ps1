...
param(
    ...
    [ValidateSet('approve_for_me', 'never')]
    [string]$ApprovalMode,
...
function Assert-InvocationShape {
    ...
    $newOnly = @('PromptFile', 'GoalObjectiveFile', 'MaxChildAgents', 'Model', 'ReasoningEffort', 'ApprovalMode', 'MaxRuntimeMinutes')
    if ($Mode -eq 'New') {
        foreach ($name in @('PromptFile', 'GoalObjectiveFile', 'MaxChildAgents', 'Model', 'ReasoningEffort', 'ApprovalMode', 'MaxRuntimeMinutes')) {
...
function Invoke-FeaturePreflight {
    ...
    Save-Manifest -Manifest $Manifest -ManifestPath $ManifestPath
    $arguments = New-CodexFeaturesArguments -RunDirectory $Manifest.run_directory -MaxChildAgents $ChildCap -ApprovalMode $ApprovalMode
...
function Resolve-CycleControllerBundle {
    $modulePath = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot 'MathResearchCycleLedgerV2.psm1'))
    $cliPath = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot 'invoke_math_research_cycle_v2.ps1'))
    $projectModulePath = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot 'MathResearchProjectArchiveV2.psm1'))
    $projectCliPath = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot 'invoke_math_research_project_v2.ps1'))
    $launcherModulePath = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot 'MathResearchLauncherV2.psm1'))
    $launcherEntryPath = [IO.Path]::GetFullPath($PSCommandPath)
    $canaryEntryPath = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot 'invoke_math_research_canary_v2.ps1'))
    $stopCliPath = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot 'stop_math_research_v2.ps1'))
    foreach ($path in @($modulePath, $cliPath, $projectModulePath, $projectCliPath, $launcherModulePath, $launcherEntryPath, $canaryEntryPath, $stopCliPath)) {
        if (-not (Test-Path -LiteralPath $path -PathType Leaf)) { throw "Required cycle-controller file is missing: $path" }
        if (-not (Split-Path -Parent $path).Equals([IO.Path]::GetFullPath($PSScriptRoot), [StringComparison]::OrdinalIgnoreCase)) { throw "V2 bundle file escaped the trusted scripts directory: $path" }
    ...
    return [pscustomobject]@{
        ...
        ProjectCliPath = $projectCliPath
        ProjectCliSha256 = Get-Sha256HexFromFile -LiteralPath $projectCliPath
        LauncherModulePath = $launcherModulePath
        LauncherModuleSha256 = Get-Sha256HexFromFile -LiteralPath $launcherModulePath
        LauncherEntryPath = $launcherEntryPath
        LauncherEntrySha256 = Get-Sha256HexFromFile -LiteralPath $launcherEntryPath
        CanaryEntryPath = $canaryEntryPath
        CanaryEntrySha256 = Get-Sha256HexFromFile -LiteralPath $canaryEntryPath
        StopCliPath = $stopCliPath
        StopCliSha256 = Get-Sha256HexFromFile -LiteralPath $stopCliPath
    }
}
...
function Import-CycleControllerBundle {
    ...
    # Resolve and hash the complete tuple before this function is called. On
    # Resume, the caller must compare it with the signed manifest before any
    # cycle/project module bytes execute.
    $projectModule = Import-Module ([string]$Bundle.ProjectModulePath) -Force -DisableNameChecking -PassThru
    $cycleModule = Import-Module ([string]$Bundle.ModulePath) -Force -DisableNameChecking -PassThru
    $required = [ordered]@{
        'Initialize-MathResearchCycleLedger' = [string]$cycleModule.Name
        'Verify-MathResearchCycleLedger' = [string]$cycleModule.Name
        'Invoke-MathResearchCycleReturnCheck' = [string]$cycleModule.Name
        'Save-MathResearchCycleCheckpoint' = [string]$cycleModule.Name
        'Test-MathResearchRouteStartObject' = [string]$projectModule.Name
    ...
    foreach ($commandName in $required.Keys) {
        ...
        if ($null -eq $command -or [string]$command.Source -cne [string]$required[$commandName]) {
            throw "V2 controller command provenance mismatch for $commandName; expected module $($required[$commandName])."
        }
    }
    return $Bundle
}
...
function Assert-CycleControllerBundleMatchesManifest {
    ...
    Assert-MathResearchV2BundleReceipt -Bundle $Bundle -Manifest $Manifest | Out-Null
}
...
function Assert-ResumeManifest {
    ...
    if ([string]$Manifest.launcher_protocol -cne 'math-research-launcher/v2' -or [string]$Manifest.prompt_version -cne 'v7') {
        throw 'versioned_migration_required: launcher v2 does not silently rewrite or Resume a v1/v3-v6 run; use the unchanged legacy Resume path or a separately authorized migration receipt.'
    }
    if ($null -eq $RunContext -or [string]$RunContext.Layout -cne 'project') { throw 'Prompt v7 may Resume only from its verified project archive runs directory.' }
    ...
    if ([string]$Manifest.config.approval_mode -notin @('approve_for_me','never')) { throw 'Manifest contains an invalid contract-bound approval mode.' }
    $hashes = @(
    ...
    $promptState = $Manifest.prompt_v7
        ...
        $storedBinding = [string]$Manifest.inputs.prompt.contract_binding_sha256
        ...
        foreach ($bundleItem in @($Manifest.launcher_bundle.Values)) { $hashes += [string]$bundleItem.sha256 }
...
try {
    ...
    if ($Mode -eq 'New') {
        ...
        $metadata = Parse-PromptV7Metadata -PromptText $promptInfo.Text
        Test-PromptMetadataAgainstParameters -Metadata $metadata -MaxChildAgents $MaxChildAgents -Model $Model -ReasoningEffort $ReasoningEffort -ApprovalMode $ApprovalMode -MaxRuntimeMinutes $MaxRuntimeMinutes -GoalObjectiveSha256 $objectiveSha256 -RunContext $runContext
        ...
        $cycleController = Resolve-CycleControllerBundle
        $cycleController = Import-CycleControllerBundle -Bundle $cycleController
        ...
        if ([int]$cycleState.TotalRoundBudget -ne [int]$metadata.total_round_budget -or
            ...
            throw 'Cycle policy budgets do not match the Prompt v7 launcher metadata.'
        }
        ...
        $manifest = [ordered]@{
            schema_version = 1
            launcher_protocol = 'math-research-launcher/v2'
            run_id = Split-Path -Leaf $runPath
            revision = 0
            created_at_utc = Get-UtcNowString
            updated_at_utc = Get-UtcNowString
            prompt_version = 'v7'
            prompt_header = 'Math Research Orchestration Prompt v7'
            ...
            launcher_bundle = [ordered]@{
                launcher_entry = [ordered]@{ path=$cycleController.LauncherEntryPath; sha256=$cycleController.LauncherEntrySha256 }
                launcher_module = [ordered]@{ path=$cycleController.LauncherModulePath; sha256=$cycleController.LauncherModuleSha256 }
                canary_entry = [ordered]@{ path=$cycleController.CanaryEntryPath; sha256=$cycleController.CanaryEntrySha256 }
                cycle_module = [ordered]@{ path=$cycleController.ModulePath; sha256=$cycleController.ModuleSha256 }
                cycle_cli = [ordered]@{ path=$cycleController.CliPath; sha256=$cycleController.CliSha256 }
                project_module = [ordered]@{ path=$cycleController.ProjectModulePath; sha256=$cycleController.ProjectModuleSha256 }
                project_cli = [ordered]@{ path=$cycleController.ProjectCliPath; sha256=$cycleController.ProjectCliSha256 }
                stop_cli = [ordered]@{ path=$cycleController.StopCliPath; sha256=$cycleController.StopCliSha256 }
            }
            inputs = [ordered]@{
            ...
            config = [ordered]@{
                ...
                approval_cli_argument = if ($ApprovalMode -eq 'approve_for_me') { '--approve-for-me' } else { '-a never' }
        ...
        $approvalCapability = Assert-CodexApprovalModeCapability -Attestation $attestation -WorkingDirectory $runPath -ApprovalMode $ApprovalMode
        $manifest.executable.approval_capability = [ordered]@{ mode=$ApprovalMode; exec_help_sha256=$approvalCapability.ExecHelpSha256; verified=$true }
        Save-Manifest -Manifest $manifest -ManifestPath $manifestPath
        Invoke-FeaturePreflight -Manifest $manifest -ManifestPath $manifestPath -Attestation $attestation -ChildCap $MaxChildAgents -ApprovalMode $ApprovalMode
        $manifest.status = 'canary_running'
        Save-Manifest -Manifest $manifest -ManifestPath $manifestPath
        $canary = Invoke-MathResearchLauncherCanaryV2 -Attestation $attestation -RunDirectory $runPath -ManifestPath $manifestPath -LauncherEntryPath $cycleController.LauncherEntryPath -LauncherModulePath $cycleController.LauncherModulePath -CanaryEntryPath $cycleController.CanaryEntryPath -CycleCliPath $cycleController.CliPath -ApprovalMode $ApprovalMode -Model $Model -ReasoningEffort $ReasoningEffort -WebSearch ([string]$metadata.web_search) -MaxChildAgents $MaxChildAgents
        $manifest.canary = [ordered]@{ protocol='math-research-launcher-canary/v2'; receipt_path=$canary.ReceiptPath; binding_sha256=$canary.BindingSha256; reused=[bool]$canary.Reused; passed=[bool]$canary.Passed }
        ...
        $goalSandbox = if ($ApprovalMode -eq 'approve_for_me') { 'workspace-write' } else { 'read-only' }
        $goalArguments = New-CodexExecArguments -RunDirectory $runPath -Model $Model -ReasoningEffort $ReasoningEffort -Sandbox $goalSandbox -ApprovalMode $ApprovalMode -AllowWebSearch:$false -EnableMultiAgent:$false -MaxChildAgents 1 -LastMessagePath $goalPaths.LastMessage -OutputSchemaPath $goalSchemaPath
        ...
        $manifest.prompt_v7.status = 'prepared'
        $manifest.prompt_v7.segment_index = [int]$manifest.next_segment_index
        $manifest.prompt_v7.submitted_sha256 = $manifest.inputs.research_turn.sha256
        ...
        $researchArguments = New-CodexExecArguments -RunDirectory $runPath -Model $Model -ReasoningEffort $ReasoningEffort -Sandbox 'workspace-write' -ApprovalMode $ApprovalMode -AllowWebSearch:$allowSearch -EnableMultiAgent:$true -MaxChildAgents $MaxChildAgents -LastMessagePath $researchPaths.LastMessage -ResumeThreadId $manifest.thread_id
        ...
        $manifest.prompt_v7.status = 'turn_completed'
        $manifest.prompt_v7.turn_completed_at_utc = Get-UtcNowString
        ...
        $approvalModeForRun = [string]$manifest.config.approval_mode
        $cycleController = Resolve-CycleControllerBundle
        Assert-CycleControllerBundleMatchesManifest -Bundle $cycleController -Manifest $manifest
        $cycleController = Import-CycleControllerBundle -Bundle $cycleController
        ...
        Save-Manifest -Manifest $manifest -ManifestPath $manifestPath
        $approvalCapability = Assert-CodexApprovalModeCapability -Attestation $attestation -WorkingDirectory $runPath -ApprovalMode $approvalModeForRun
        $manifest.executable.approval_capability = [ordered]@{ mode=$approvalModeForRun; exec_help_sha256=$approvalCapability.ExecHelpSha256; verified=$true }
        Save-Manifest -Manifest $manifest -ManifestPath $manifestPath
        Invoke-FeaturePreflight -Manifest $manifest -ManifestPath $manifestPath -Attestation $attestation -ChildCap ([int]$manifest.config.max_child_agents) -ApprovalMode $approvalModeForRun
        $manifest.status = 'canary_running'
        Save-Manifest -Manifest $manifest -ManifestPath $manifestPath
        $canary = Invoke-MathResearchLauncherCanaryV2 -Attestation $attestation -RunDirectory $runPath -ManifestPath $manifestPath -LauncherEntryPath $cycleController.LauncherEntryPath -LauncherModulePath $cycleController.LauncherModulePath -CanaryEntryPath $cycleController.CanaryEntryPath -CycleCliPath $cycleController.CliPath -ApprovalMode $approvalModeForRun -Model ([string]$manifest.config.model) -ReasoningEffort ([string]$manifest.config.reasoning_effort) -WebSearch ([string]$manifest.config.web_search) -MaxChildAgents ([int]$manifest.config.max_child_agents)
        $manifest.canary = [ordered]@{ protocol='math-research-launcher-canary/v2'; receipt_path=$canary.ReceiptPath; binding_sha256=$canary.BindingSha256; reused=[bool]$canary.Reused; passed=[bool]$canary.Passed }
        $manifest.status = 'canary_passed'
        Save-Manifest -Manifest $manifest -ManifestPath $manifestPath
        ...
        $manifest.inputs.continuations = @($manifest.inputs.continuations) + @([ordered]@{
            ...
            wrapper = "launcher_enforced_cycle_continuation_gate_v2:$resumeCycleMode"
        })
    ...
    if ($manifest -and $manifestPath) {
        try {
            if ($cycleController -and $manifest.prompt_version -eq 'v7' -and $null -ne $manifest.cycle_ledger) {
                try {
