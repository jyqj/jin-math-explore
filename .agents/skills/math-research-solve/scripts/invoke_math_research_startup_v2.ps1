...
function Invoke-ControllerAction {
    ...
    $controller = Join-Path $PSScriptRoot 'invoke_math_research_project_v2.ps1'
    ...
    $command = "& $(& $quote $observer) -Skill 'math-research-solve' -Catalog 'math-research-solve/v1' -Phase 'math-research-solve.script.invoke_math_research_project_v2' -FilePath $(& $quote $controller) -ArgumentList @($encodedArguments)"
...
if ($null -eq $startupClass) {
    ...
    if ([string]$projectHint.status -eq 'migration_required' -or $migrationHint -notin @('not_required','verified','complete')) {
...
$clock.Stop()
[ordered]@{
    schema = 'math-research-startup-plan/v2'
    ok = $true
