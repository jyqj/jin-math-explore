...
try {
    ...
    if ($Mode -eq 'New') {
        ...
        $compatCanaryHostPath = Join-Path $PSScriptRoot 'invoke_math_research_legacy_v1_compat_canary_host.ps1'
        $installedCanaryEntryPath = Join-Path $PSScriptRoot 'invoke_math_research_canary_v2.ps1'
        Assert-MathResearchLegacyV1CompatState -Manifest $manifest -RunPath $runPath -ReceiptRead $receiptRead -LauncherEntryPath $PSCommandPath -LauncherModulePath $compatLauncherModulePath -CycleModulePath $compatCycleModulePath -CycleCliPath $compatCycleCliPath -ProjectModulePath $compatProjectModulePath -CanaryHostPath $compatCanaryHostPath -CanaryEntryPath $installedCanaryEntryPath -RequireApplied | Out-Null
        ...
        $canaryOutput = & $pwsh -NoLogo -NoProfile -File $compatCanaryHostPath -RunDirectory $runPath -ManifestPath $manifestPath
        ...
        $manifest.compatibility_migration['canary'] = [ordered]@{ protocol='math-research-launcher-canary/v2'; receipt_path=[string]$canary.ReceiptPath; binding_sha256=[string]$canary.BindingSha256; reused=[bool]$canary.Reused; passed=[bool]$canary.Passed }
